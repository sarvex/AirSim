...
client.simPlotStrings(
    strings=["Microsoft AirSim" for _ in range(5)],
    positions=[
        Vector3r(x, y, -1)
...
client.simPlotTransformsWithNames(
    ...
    names=[f"tf_yaw_{str(idx)}" for idx in range(10)],
    tf_scale=35,
    tf_thickness=5,
    text_scale=1,
...
client.simPlotTransformsWithNames(
    ...
    names=[f"tf_roll_{str(idx)}" for idx in range(10)],
    tf_scale=35,
    tf_thickness=5,
    text_scale=1,
    text_color_rgba=[1.0, 1.0, 1.0, 1.0],
...
client.simPlotTransformsWithNames(
    poses=[
        Pose(
            position_val=Vector3r(x, y, 0),
            orientation_val=to_quaternion(pitch=pitch, roll=0.0, yaw=0.0),
        )
        for x, y, pitch in zip(
            np.linspace(0, 10, 10),
            np.linspace(-1, -1, 10),
            np.linspace(0, np.pi, 10),
        )
    ],
    names=[f"tf_pitch_{str(idx)}" for idx in range(10)],
    tf_scale=35,
    tf_thickness=5,
    text_scale=1,
    text_color_rgba=[1.0, 1.0, 1.0, 1.0],
